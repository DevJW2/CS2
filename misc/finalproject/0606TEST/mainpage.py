...
theloggedin = f3.read()
f3.close()
splitlogged = theloggedin.split("\n")
moresplitlogged = []
for loggedin in splitlogged:
	moresplitlogged.append(loggedin.split(","))
moresplitlogged.pop()

#f5 = open("../data/favorites.txt", "w")
#f5.write("")
#f5.close()
...
if 'HTTP_COOKIE' in os.environ:
	...
	if 'username' in c and 'ID' in c:
		...
		if authenticate(username,ID,IP): # change here
			splitfavorites = thefavoritesread.split("\n")
			totalusersfavorites = []
			for totalusers in splitfavorites: 
				totalusersfavorites.extend(totalusers)
			body += "<h1>" + str(splitfavorites) + "</h1>"
			temp_list = []
			temp_user = []
			temp_write = ""
				...
				for fav in formresults: 
					if formresults.getvalue(fav) == "favorite" and fav not in thefavoritesread:
						if os.stat("../data/favorites.txt").st_size == 0: 
							body += "<h1> this is working </h1>"
							body += str(moresplitlogged)
							splitfavorites.pop()
							f2 = open("../data/favorites.txt", "a")
							splitfavorites.append(str(moresplitlogged[-1][0]) + "," + str(fav))
							f2.write(str(splitfavorites) + "\n")
							f2.close()
						elif moresplitlogged[-1][0] not in totalusersfavorites:
							body += "<h1> this second elif is working </h1>"
							f2 = open("../data/favorites.txt", "a")
							temp_user.append(str(moresplitlogged[-1][0]) + "," + str(fav))
							f2.write(str(temp_user) + "\n")
							f2.close()
						else: 
							body += "<h1> this third elif is working </h1>"
							for x in splitfavorites: 
								if x == "": 
									break
								elif x[0] == moresplitlogged[-1][0]:
									x[:].append(str(fav))
