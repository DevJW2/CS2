...
if len(results)>0:
    if results.getvalue('name')+',' in restaurants:
        ...
        f.write(results.getvalue('name')+','+results.getvalue('cost')+','+results.getvalue('distance')+','+results.getvalue('food')+','+results.getvalue('rate')+ ',' + results.getvalue('description') + '\n')
        f.close()
